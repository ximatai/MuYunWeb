...
service.interceptors.request.use(
  (config) => {
    ...
    //base64转换condition参数
    if (
      (config.url.substring(config.url.length - 10) == '/reference' ||
        config.url.substring(config.url.length - 5)) == '/view' &&
      typeof config.params != 'undefined' &&
      typeof config.params.condition != 'undefined' &&
      config.params.condition.charAt(config.params.condition.length - 1) != '='
    ) {
      typeof lr != null
        ? (config.params.condition =
            PREFIX + Base64.encode(config.params.condition) + '=')
        : (config.params.condition =
            Base64.encode(config.params.condition) + '=')
      config.params.condition =
        config.params.condition.split('').reverse().join('') + '@'
    }
    if (
      (config.url.substring(config.url.length - 10) == '/reference' ||
        config.url.substring(config.url.length - 5)) == '/view' &&
      typeof config.data != 'undefined' &&
      typeof config.data.condition != 'undefined' &&
      config.data.condition.charAt(config.data.condition.length - 1) != '='
    ) {
      typeof lr != null
        ? (config.data.condition =
            PREFIX + Base64.encode(config.data.condition) + '=')
        : (config.data.condition = Base64.encode(config.data.condition) + '=')
      config.data.condition =
        config.data.condition.split('').reverse().join('') + '@'
    }
